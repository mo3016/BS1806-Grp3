...
First we randomized the data set using sample(), and stored the result in df_rand.
...
```
The default observation $\frac{Num\ of\ loans\ repaid}{Num\ of\ total\ loans}$ is = 85.9%

Once we had run the classification model on the validation set we achieved an accuracy of 85.8%. This is marginally lower then our default value, but not significantly different. 
...
This primitive model, which uses 'majority votes' is not very useful on this particular data set. Due to the small number of 'charged off' samples. We can increase the effectiveness of this model by adapting it to put more weight on certain errors.
...
Using the cost matrix above, we achieved a sensitivity of 25%. The precision is 23.7%, this is the percentage of loans that we would recommend to the bank for re-evaluation that were indeed charged off.
...
Using the cost matrix above, we achieved a sensitivity of 40%. The precision is 21.3%, this is the percentage of loans that we would recommend to the bank for re-evaluation that were indeed charged off.
...
Using the cost matrix above, we achieved a sensitivity of 50%. The precision is 21.1%, this is the percentage of loans that we would recommend to the bank for re-evaluation that were indeed charged off.
...
## 2.4)
An increased sensitivity level increases the prediction rate of our important class (charged off), however it leads to more false positive classifications too. This means that there are both advantages and disadvantages to choosing the model with the highest sensitivity or precision alone.

Economically, We feel that a False Negative classification would be most costly to a bank. This would be a situation when a customer is classified as being able to repay their loan, but in fact defaults on that loan. For this reason, and in the interest of caution, we feel that a higher sensitivity value is more beneficial to the bank. In addition, this will need to be balanced against a high proportion of charged off loans being classified by our model and in turn the resources of reviewing more applications with potential of being fully repaid. 

From the above models, we would recommend the 40% sensitivity model, which has a precision of 21.3%. The cost matrix for this model can be seen above. The model reduces the number of False Negatives whilst maintaining a reasonable level of precision.
...
This model predicts 26% of customers to be 'charged off' vs. an actual value of 14%. It also predicts 73.4% of customers to be 'fully paid' vs. an actual value of 85.9%. We are slightly overestimating the number of 'charged off' customers, however this would be an appropriate risk averse strategy for a bank to follow,